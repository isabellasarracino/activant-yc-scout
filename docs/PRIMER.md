...
5. Sanity check: the real repo has 13 commits and 172 passing tests as of this writing (see "Current build status" below). If a fresh `git log` or `npm test` in the new chat shows meaningfully less than that, the upload is probably incomplete — ask before assuming this primer is out of sync with the code.
...
**172 tests passing, `tsc --noEmit` clean, `next build` succeeds**, as of the last commit. Full file map:
...
tests/               27 files, 172 tests — tests/fixtures/ has real captured YC data + an in-memory fake DB; tests/api/ exercises route handlers directly; tests/frontend/ uses React Testing Library + jsdom for every dashboard + chat component (no real browser available in this sandbox — see Known gaps)
```
...
2. **`scoreTriage` and `scoreDeepDive`** (`src/lib/scoring/triage.ts`, `deepDive.ts`) — **unverified again, new cause**. Both were confirmed working against the real Anthropic API earlier (the Summer 2026 batch actually scored, including at least one company that completed the old web-search deep-dive path). Both were then fully rewritten to call OpenRouter's OpenAI-compatible endpoint instead — different request shape, different response shape, different model slugs (`anthropic/claude-sonnet-5` etc., see `MODELS` in `src/lib/ai/openrouter.ts`). Unit-tested against a mocked `openai` client; not one real call yet. Also: deep-dive no longer does live web search at all (dropped, not replicated — see the Decisions table) — it's now structurally identical to triage plus the company's own website in the prompt, not a multi-turn agentic loop.
...
4. Scoring/extraction/chat models are now OpenRouter slugs (`anthropic/claude-sonnet-5`, `anthropic/claude-haiku-4.5`), overridable via `OPENROUTER_SCORING_MODEL` / `OPENROUTER_EXTRACTION_MODEL` / `OPENROUTER_CHAT_MODEL`. These were chosen from web search results showing them available on OpenRouter around the time this was written, **not confirmed by an actual successful call** — the single most likely thing to need a quick fix on first real use if a request 404s.
...
8. **Pipeline resilience** (`runBatchPipeline.ts` catching per-company failures, `scoreTool.ts`'s malformed-input validation, `triage.ts`/`deepDive.ts`'s truncated-response guards) — built and unit-tested (the exact failure mode that motivated this — a malformed `record_score` call crashing the whole batch — is covered by a specific regression test), but not yet re-confirmed against OpenRouter specifically. The original incident happened on direct Anthropic; the fix is provider-agnostic in principle (it's about handling a bad response, not about which provider sent it), but "in principle" isn't the same as watched-it-work.
